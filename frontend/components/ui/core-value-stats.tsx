...
export default function CoreValueStats({
  ...
  return (
    <section className=" mx-auto px-6 text-center bg-black">
      {/* Section header */}
      ...
      <div className="flex flex-nowrap overflow-x-auto gap-6 mt-10 sm:flex-wrap sm:justify-center">
        {stats.map((item, i) => {
            ...
            <CardContent className="relative z-10 p-6 space-y-3 text-left flex flex-col justify-end h-full">
              ...
              <Button
                ...
                className={`px-0 text-sm font-medium mt-2 ${
                  item.image ? "text-white hover:text-gray-200" : "text-blue-400 hover:text-blue-300"
                }`}
              >
                Learn more →
              </Button>
          ...
          return (
            ...
            >
              <Card className="relative h-64 overflow-hidden border border-gray-800 bg-gray-900 shadow-sm hover:shadow-lg transition text-white rounded-3xl">
                {cardContent}
              </Card>
            </motion.div>
          );
